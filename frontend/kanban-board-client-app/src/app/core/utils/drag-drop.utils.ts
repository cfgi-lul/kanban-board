...
export function handleDragDrop(
  ...
  if (isSamePosition) {
    ...
    const previousColumn = currentBoard.columns?.find(
      col => col.id?.toString() === previousContainer.id
    );
    const currentColumn = currentBoard.columns?.find(
      col => col.id?.toString() === container.id
    );
  ...
  const previousColumn = updatedBoard.columns?.find(
    col => col.id?.toString() === previousContainer.id
  );
  const currentColumn = updatedBoard.columns?.find(
    col => col.id?.toString() === container.id
  );
  ...
  if (previousContainer === container) {
    ...
    const sortedTasks = [...currentColumn.tasks].sort(
    ...
    if (sortedTasks.length === 0) {
      ...
      if (beforeTask && afterTask) {
        newPosition =
          ((beforeTask.position ?? 0) + (afterTask.position ?? 0)) / 2;
      } else {
